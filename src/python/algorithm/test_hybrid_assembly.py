...
if __name__ == "__main__":
    ...
    parser.add_argument("case", action="store", help="case identifier suffix")
    parser.add_argument(
        "-i",
        "--initialize",
        action="store_true",
    ...
    parser.add_argument(
        "-w", "--use-wgsim", action="store_true", help="use wgsim to simulate reads"
    )
    parser.add_argument(
        "-s",
    ...
    if not os.path.exists(pickle_file_name) or options.initialize:
        ...
        a_seq = utilities.create_r_seq(int((SEQ_LEN - rep_len) / 2))
        if options.embed_repeat:
            b_seq, r_k_mers = utilities.create_r_seq_w_rep(
                k_mer_len=K_MER_LEN, k_mer_cnt=K_MER_CNT_REP
            )
        else:
            b_seq = utilities.create_r_seq(rep_len)
            r_k_mers = []
        c_seq = utilities.create_r_seq(SEQ_LEN - len(a_seq) - len(b_seq))
        ...
        if options.use_wgsim:
            ...
            print(
                "Simulating paired reads from random sequence ...", end=" ", flush=True
            )
        ...
        start_time = time.time()
        print(
            "Simulating unpaired reads from repeat containing sequence ...",
            end=" ",
            flush=True,
        )
        rd_file_name_q, rd_file_name_a = utilities.simulate_unpaired_reads(
            d_seq, case_file_name, number_pairs=NUMBER_READS, len_read=LEN_UNPAIRED_READ
        )
        print("done in {0} s".format(time.time() - start_time), flush=True)
    ...
    if options.assemble_using_spades and (
        ...
        if options.include_long_reads:
            ...
            command = utilities.spades(rd1_file_name, rd2_file_name, spades_out_dir)
        print("done in {0} s".format(time.time() - start_time), flush=True)
    ...
    if os.path.exists(spades_out_dir):
        spades_seq_rcds = [
            ...
            for seq_record in SeqIO.parse(
                os.path.join(spades_out_dir, "contigs.fasta"), "fasta"
            )
        ]
    ...
    if options.assemble_using_unicycler and (
        ...
        if options.include_long_reads:
            ...
            command = utilities.unicycler(
                rd1_file_name, rd2_file_name, unicycler_out_dir
            )
        print("done in {0} s".format(time.time() - start_time), flush=True)
        print("Command: {0}".format(command), flush=True)
    ...
    if os.path.exists(unicycler_out_dir):
        unicycler_seq_rcds = [
            seq_record
            for seq_record in SeqIO.parse(
                os.path.join(unicycler_out_dir, "assembly.fasta"), "fasta"
            )
        ]
